...
export const PillBase: React.FC = () => {
    ...
    // Detect active section on mount and path change
    useEffect(() => {
    ...
    useEffect(() => {
        if (hovering) {
            setExpanded(true)
            const newWidth = typeof window !== 'undefined' ? Math.min(580, window.innerWidth - 32) : 580
            pillWidth.set(newWidth)
